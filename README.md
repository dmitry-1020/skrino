...
Во время записи появляется компактная панель управления: красная точка, таймер, пауза, «Стоп» и крестик отмены. Панель не попадает в саму запись (исключена из захвата), всегда поверх других окон и её можно перетаскивать. Под таймером показан хоткей остановки, потому что повторное нажатие хоткея записи тоже завершает её. При записи области панель встаёт под выделенной областью, если там есть место. При записи всего экрана панель уходит на второй монитор, если он есть, иначе прижимается к нижнему центру экрана (над панелью задач), чтобы не закрывать содержимое. В покое панель полупрозрачная, при наведении курсора становится полностью непрозрачной. Границы записываемой области отмечены тонкой жёлтой рамкой (тоже не попадает в видео).

Куда уходит готовый MP4, определяется тем же разделом «Поделиться», что и для скриншотов: в локальную папку (учитывается «Спрашивать место при каждом сохранении», диалог с фильтром `.mp4`) или на сервер по FTP/FTPS/SFTP, после чего ссылка копируется в буфер и приходит системное уведомление. Настройки записи (частота кадров 15/30/60, запись курсора, звук, хоткеи) находятся в разделе «Запись».

**Звук.** В запись можно добавить один источник звука: «Системный звук» (то, что играет на компьютере) или «Микрофон». Выбирается в настройках в разделе «Запись», по умолчанию звук выключен. Одновременно пишется только один источник, смешивания системного звука и микрофона нет. Учтите: если системный звук выводится через колонки, а не наушники, микрофон может его подхватить, и в записи получится эхо.

**Ограничения записи:** пишется только один монитор (тот, где выбрана область; для полноэкранной записи это монитор под курсором); звук опционален (системный или микрофон), по умолчанию выключен.
