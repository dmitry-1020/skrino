...
pub fn top_toolbar(state: &mut EditorState, ui: &mut egui::Ui, palette: &Palette) {
    ...
    ui.horizontal_centered(|ui| {
        ui.add_space(4.0);
        let tools: [(Tool, &str, &str); 11] = [
            (Tool::Select, ph::CURSOR, "Выделение"),
            ...
            (Tool::Counter, ph::NUMBER_CIRCLE_ONE, "Счётчик"),
            (Tool::Crop, ph::CROP, "Обрезать"),
        ];
        for (tool, icon, tip) in tools {
            let selected = state.tool == tool;
            if icon_button(ui, palette, icon, tip, selected, true).clicked() {
                state.set_tool(tool);
            }
        }
...
pub fn bottom_bar(
    ...
    ui.horizontal_centered(|ui| {
        ...
        if ui
            .add(egui::Button::new(RichText::new(format!("{}  По размеру окна", ph::ARROWS_OUT))))
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            let share_btn = egui::Button::new(RichText::new(share_label).color(Color32::WHITE))
